            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between mt-10">
                <div>
                    <p className="text-sm text-gray-700">
                        Visualizzati da <span className="font-medium">{(page - 1) * limit + 1}</span> a <span className="font-medium">{Math.min(page * limit, tours.total)}</span> di{' '}
                        <span className="font-medium">{tours.total}</span> risultati
                    </p>
                </div>
                <div>
                    <nav aria-label="Pagination" className="isolate inline-flex -space-x-px rounded-md shadow-sm">
                        ...
                        {Array.from({ length: tours.totalPages }, (_, index) => index + 1).map((pageNumber) => (
                            <button
                                key={pageNumber}
